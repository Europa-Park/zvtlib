	...
	public class ReportStarter
	{
		...
		public static void Main(string[] args){
			...
			if(args[0].Equals("?")){
					...
					Console.WriteLine("report <config.xml>");
					printHelp();
					return;
			}
			...
			try{
				...
				CommandResult result = environment.CreateReportCommand((XmlElement)config.DocumentElement.SelectSingleNode("Report")).Execute();
				//create XML file with result message
		  ...
		  static void environment_StatusReceived(IntermediateStatus status)
        {
			...
			Console.WriteLine("The report config.xml needs to look like this:");
			Console.WriteLine("<Config>");
			Console.WriteLine("		<Transport>....</Transport>");
			Console.WriteLine("		<TransportSettings>.....</TransportSettings>");
			Console.WriteLine("		<Report>");
			Console.WriteLine("			<PrintSystemInfo>...</PrintSystemInfo>");
			Console.WriteLine("			<PrintReport>...</PrintReport>");
			Console.WriteLine("		</Report>");
			Console.WriteLine("</Config>");
		}
	}
